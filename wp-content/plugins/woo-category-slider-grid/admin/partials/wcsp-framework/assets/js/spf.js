...
; (function ($, window, document, undefined) {
    ...
    return this.each(function () {
      ...
      $this.on({
        mouseenter: function () {
          ...
          var help_text = $this.find('.spf-help-text').html();
          if ($('.spf-tooltip').length > 0) {
            $tooltip = $('.spf-tooltip').html(help_text);
          } else {
            $tooltip = $('<div class="spf-tooltip ' + $class + '"></div>').html(help_text).appendTo('body');
          }

          offset_left = SP_WCS.vars.is_rtl
            ? $this.offset().left + 33
            : $this.offset().left + 33
          ...
          if ($this.find('.spf-support').length > 0) {
            $top = $this.offset().top + 53;
            offset_left = $this.offset().left - 213;
          }
  ...
  var lastSelectedOption = $('input[name="sp_wcsp_shortcode_options[wcsp_layout_presets]"]:checked').val();
  $('input[name="sp_wcsp_shortcode_options[wcsp_layout_presets]"]').change(function () {
    if (!$(this).is(':disabled')) {
  ...
  $('#publishing-action').on('click', '#publish', function (e) {
    if ($('input[name="sp_wcsp_shortcode_options[wcsp_layout_presets]"]:checked').is(':disabled')) {
      $('input[name="sp_wcsp_shortcode_options[wcsp_layout_presets]"][value="' + lastSelectedOption + '"]').prop('checked', true);
    }
  });
