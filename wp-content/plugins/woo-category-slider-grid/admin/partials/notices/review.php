 ...
 * The Review class.
 *
 * This is used to review related functionalities.
 */
class Woo_Category_Slider_Review {
	...
	public function dismiss_review_notice() {
		...
		$notice_dismissed_data = isset( $_POST['notice_dismissed_data'] ) ? sanitize_text_field( wp_unslash( $_POST['notice_dismissed_data'] ) ) : '';
	...
	public function woo_product_slider_install_admin_notice() {
		...
		if ( current_user_can( 'install_plugins' ) && 'sp_wcslider' === $the_current_post_type ) {
			...
			if ( in_array( 'woo-product-slider/main.php', $plugins, true ) ) {
				...
				$install_url = esc_url( self_admin_url( 'plugins.php?action=activate&plugin=' . rawurlencode( 'woo-product-slider/main.php' ) . '&plugin_status=all&paged=1&s&_wpnonce=' . rawurlencode( wp_create_nonce( 'activate-plugin_woo-product-slider/main.php' ) ) ) );
			}
	...
	public function smart_brand_install_admin_notice() {
		...
		if ( current_user_can( 'install_plugins' ) && 'sp_wcslider' === $the_current_post_type ) {
			...
			// $arrow variable is ignore escaping because of svg icon.
			echo sprintf( '<div class="wsb-notice notice is-dismissible" data-nonce="%7$s"><img src="%1$s"/><div class="wsb-notice-text">To <strong>Add Brands</strong> and <strong>Highlight the Brands of the Products</strong> you sell, %4$s the <a href="%2$s" class="thickbox open-plugin-details-modal"><strong>Smart Brands for WooCommerce</strong></a> <a href="%3$s" rel="noopener" class="wsb-activate-btn">%5$s</a><a href="https://demo.shapedplugin.com/smart-brands-for-woocommerce/" target="_blank" class="wsb-demo-button">See How It Works<span>%6$s</span></a></div></div>', esc_url( $icon ), esc_url( $popup_url ), esc_url( $install_url ), esc_html( $text ), esc_html( $button_text ), $arrow, esc_attr( $nonce ) ); // phpcs:ignore
		}
	}
